...
def co_occurrences(wordsequence, window=2):
    ...
    for index in range(len(wordsequence) - 1):
        ...
        if index + window < len(wordsequence):
            # there is a word 2 words away
            # count that word[index] as a target co-occurred with the but-next word as a context item,
            # and vice versa
            target_context_pairs.append( (wordsequence[index], wordsequence[index+window]) )
            target_context_pairs.append( (wordsequence[index+window], wordsequence[index]) )
...
def make_space(demo_dir, word_index, numdims, window=2):
    ...
    for filename in os.listdir(demo_dir):
        if filename.endswith("txt"):
            ...
            for sentence in sentences:
                ...
                for target, cxitem in co_occurrences(words, window):
